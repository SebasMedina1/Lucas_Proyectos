...
try {
    ...
    $filename = sprintf('libro_ventas_%s_%s.csv', str_replace('-','',$desde), str_replace('-','',$hasta));
    ...
    // Headers
    fputcsv($output, ['Fecha', 'Tipo', 'N° Documento', 'Timbrado', 'Cliente', 'RUC', 'Exento', 'Base 5%', 'IVA 5%', 'Base 10%', 'IVA 10%', 'Total', 'Estado'], ';');
    ...
    foreach ($documentos as $doc) {
        ...
        fputcsv($output, [
            ...
            $doc['cliente'],
            $doc['ruc'],
            $signo . number_format($doc['exento'] * $doc['signo'], 0, ',', '.'),
            $signo . number_format($doc['base_5'] * $doc['signo'], 0, ',', '.'),
            $signo . number_format($doc['iva_5'] * $doc['signo'], 0, ',', '.'),
            $signo . number_format($doc['base_10'] * $doc['signo'], 0, ',', '.'),
            $signo . number_format($doc['iva_10'] * $doc['signo'], 0, ',', '.'),
            $signo . number_format($doc['total'] * $doc['signo'], 0, ',', '.'),
            $estadoLabel
        ], ';');
    }
